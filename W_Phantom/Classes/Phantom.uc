...
class Phantom extends ASVehicle placeable;
...
var() float	MaxLandingSpeed; // Plane lands if collision slower than this
var() bool  bPlaneLanded;
...
/*
    ...
    if ( VSize(Velocity) < 210 ){
        Log("VSize Velocity:["$VSize(Velocity)$"] Velocity:["$Velocity$"]");
        if(!bPlaneLanded){
            PlaneLanded();
        }
        if(bGearsDown==False){
...
simulated function PlaneTakeOff(){
...
simulated function PlaneLanded(){
...
*/
	...
	if(bLanded ==true) {
       //log("UpdateRocketAcceleration:bLanded?");
        ...
        //log("UpdateRocketAcceleration:bLanded? NOT");
    }
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, class<DamageType> damageType)
{
    ...
    log("TakeDamage:instigatedBy-"$instigatedBy);
...
simulated function HitWall(vector HitNormal, actor Wall)
{
    ...
    // low air speed
    log("Landing Speed:"$VSize(Velocity));
    if( VSize(Velocity) < MaxLandingSpeed){

        // comming at ground slowly (1/6th of max landing speed)
        log("Altitude Speed:"$Velocity.Z);
        if( Velocity.Z < 0 && ABS(Velocity.Z) < (MaxLandingSpeed/6)){

            // Hit ground and not a wall
            log("Hitting Ground"$HitNormal.Z);
            if(HitNormal.Z == 1.0){

                // Plan in landing orentation (not upside down)
                // if ...
                PlaneLanded();
                return;
            }
...
//         PlaySound(sound'W_Phantom-SD.NKRadExp',SLOT_None,2.5);
...
defaultproperties
{
     ...
     FlybySound=Sound'W_Phantom-SD.PhantomFlyby'
     Flybyoffset=(Z=-60.000000)
     FlybyInterval=6.500000
     FlybyTraceDistance=4000.000000
     LaunchSound=Sound'W_Phantom-SD.EnginesLightupA'
