class EMPRedeemerWarhead extends Pawn;
...
var Vehicle AffectedVehicle;
var EMPTimer EMPTimerDisabler;
...
//        E = Spawn(class'RedeemerExplosion',,, HitLocation - 100 * Normal(Velocity), Rot(0,16384,0));
        E = Spawn(class'NewIonEffect',,, HitLocation - 100 * Normal(Velocity), Rot(0,16384,0));
...
//		Spawn(class'IonCore',,, Location, Rotation);
        Spawn(class'NewIonEffect',,, Location, Rotation);
		if ( SmokeTrail != None )
			SmokeTrail.Destroy();
		ShakeView();
    }
...
Begin:
    ...
    EMPTimerDisabler = spawn(class'EMPTimer');
    EMPTimerDisabler.SetTimer(1,true);
    foreach AllActors( class 'Vehicle', AffectedVehicle )
    {
        if ( VSize(Location - AffectedVehicle.Location) < DamageRadius){
            EMPTimerDisabler.AddVehicle(AffectedVehicle);
        }
    }
