...
//
//-----------------------------------------------------------
class WumpusASWeaponPawn extends Vehicle;
//	native
//	nativereplication;
...
var WumpusASVehicle          VehicleBase;
...
function BeginPlay()
{
	Super.BeginPlay();
        ...
        if (Gun != None)
        {
            Log("ASWeaponPawn:BeginPlay Gun:"$string(GunClass));
        	PitchUpLimit = Gun.PitchUpLimit;
        	PitchDownLimit = Gun.PitchDownLimit;
        }else{
            Log("ASWeaponPawn:BeginPlay Gun is None");
        }
}
...
function AttachToVehicle(WumpusASVehicle VehiclePawn, name WeaponBone)
{
