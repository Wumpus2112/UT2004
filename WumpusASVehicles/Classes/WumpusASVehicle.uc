...
//
//-----------------------------------------------------------
class WumpusASVehicle extends ASVehicle abstract;
...
{
    var()           class<WumpusASWeaponPawn>            WeaponPawnClass;
    var()           name                            WeaponBone;
};
var(SVehicle)       array<PassengerWeaponStruct>    PassengerWeapons;
var                 array<WumpusASWeaponPawn>            WeaponPawns;
...
simulated function PostNetBeginPlay()
{
    ...
    if (Role == ROLE_Authority)
    {
        ...
        for(i=0;i<DriverWeapons.Length;i++)
        {
            ...
            log(" Weapon: ["$string(i)$"]:["$Weapons[i].Name$"]:");
...
{
 log(" ClientRegisterVehicleWeapon: ["$string(Index)$"]:["$W$"]:");
	if (W == None)
...
{
 log(" ServerVerifyVehicleWeapon: ["$string(Index)$"]:["$Weapons[Index]$"]:");
...
function Vehicle FindEntryVehicle(Pawn P)
{
	...
	local WumpusASWeaponPawn ClosestWeaponPawn;
	local Bot B;
...
simulated function DrawHUD(Canvas Canvas)
{
	...
	{
	    log("WumpusASVehicle:DrawHUD- PC != none && !PC.bBehindView && HUDOverlay != None;");

        if (!Level.IsSoftwareRendering())
        {
    	    log("WumpusASVehicle:DrawHUD-!Level.IsSoftwareRendering();");
	...
	//    log("WumpusASVehicle:DrawHUD-ActivateOverlay(False);");
    }
}
