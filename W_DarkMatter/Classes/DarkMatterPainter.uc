class DarkMatterPainter extends Weapon;
...
var DarkMatterCannon FirstCannon;
...
simulated event RenderOverlays( Canvas Canvas )
{
    ...
    local DarkMatterFire PainterFire;
    ...
    {
		PainterFire = DarkMatterFire(FireMode[0]);
...
function float GetAIRating()
{
	...
	if ( B.IsShootingObjective() && Trace(HitLocation, HitNormal, B.Target.Location - vect(0,0,2000), B.Target.Location, false) != None )
	{
		...
		if ( CheckMarkDM(MarkLocation, false) != None )
			return AIRating;
	}
...
function DarkMatterCannon CheckMarkDM(vector MarkLocation, bool bFire)
{
	if ( FirstCannon != None )
		return FirstCannon;
	foreach DynamicActors(class'DarkMatterCannon', FirstCannon)
		return FirstCannon;
	FirstCannon = spawn(class'DarkMatterCannon',,,MarkLocation+vect(0,0,2000));
	return FirstCannon;
}
...
defaultproperties
{
     ...
     FireModeClass(0)=Class'W_DarkMatter.DarkMatterFire'
     ...
     bNotInDemo=True
     Description="The Dark Matter Painter seems innocuous enough at first glance, emitting a harmless low-power laser beam when the primary firing mode is engaged. Several seconds later a orbital dark matter cannon fires on the target, creating a temporary area of reverse gravity.||The Dark Matter Painter is a remote targeting device used to orient and fire the ONYX Dark Matter Cannon. The Dark Matter Painter offers increased targeting accuracy via its telescopic sight, easily activated by the secondary fire mode of the device.|Once the Dark Matter Painter has been used to designate a target it is highly recommended that the user put considerable distance between themselves and the devices's area of effect."
     ...
     InventoryGroup=0
     PickupClass=Class'W_DarkMatter.DarkMatterPickup'
     ...
     ItemName="Dark Matter Device Painter"
