class DarkMatterFire extends WeaponFire;

var DarkMatterPainterBeamEffect Beam;
var float UpTime;
var bool bDoHit;
var bool bValidMark;
...
var DarkMatterCannon IonCannon;
var float MarkTime;
...
state Paint
{
	function Rotator AdjustAim(Vector Start, float InAimError)
	{
		...
		if ( Bot(Instigator.Controller) != None )
		{
			...
			if ( bAlreadyMarked )
				...
				Instigator.Controller.FocalPoint = DarkMatterPainter(Instigator.Weapon).MarkLocation;
    ...
    function BeginState()
    {
        if (Weapon.Role == ROLE_Authority)
        {
            if (Beam == None)
            {
                Beam = Weapon.Spawn(class'DarkMatterPainterBeamEffect');
            }
            bInitialMark = true;
            bValidMark = false;
    ...
    function ModeTick(float dt)
    {
                ...
                if (Other.bWorldGeometry)
                {
                    if (VSize(HitLocation - MarkLocation) < 50.0)
                    {
                        ...
                        if (Level.TimeSeconds - MarkTime > 0.3)
                        {
                            ...
                            if ( IonCannon == None )
								IonCannon = DarkMatterPainter(Weapon).CheckMarkDM(HitLocation, bEngageCannon);

                            if ( (IonCannon != None) &&  IonCannon.CheckMarkDM(Instigator,HitLocation,bEngageCannon))
                            {
                                if ( IonCannon.IsFiring() )
                                {
									Instigator.PendingWeapon = None;
                                    DarkMatterPainter(Weapon).ReallyConsumeAmmo(ThisModeNum, 1);
        ...
        DarkMatterPainter(Weapon).EndEffect = EndEffect;
