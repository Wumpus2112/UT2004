...
class Fighter_Base extends ASVehicle
    abstract
    config(User);
...
{
    AirSpeed=2000.0; // hack
    super.PossessedBy(C);

    // Don't start at full speed
    //Velocity = EngineMinVelocity * vector(Rotation);

    Velocity = AirSpeed * Vector(Rotation); // hack
    Acceleration = Velocity;
    DesiredVelocity = 2000.0 ; //hack
...
simulated function VehicleCollision(Vector HitNormal, Actor Other)
{
    ...
    if ( Damage > 1.f )
    {
        //Damage      *= Damage; // damage hack
        Inst        = Pawn(Other);

        // Force death with other Pawn's collision
//        if ( Instigator != None ) Damage += HealthMax; ?any collision kills?
        log("Normal Speed:"$NormalSpeed$" Velocity:"$Velocity$" EngineMinVelocity:"$EngineMinVelocity$" Inst:"$Inst);
...
//    EngineMinVelocity=600.0
    EngineMinVelocity=0.0  // hack
//    AirSpeed=2000.0
    AirSpeed=0.000000      // hack
